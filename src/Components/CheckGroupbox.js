...
export const CheckGroupbox = ({
    ...
    return (
        <View style={{ marginTop: 5 }}>
            ...
            <View style={{
                ...commonStyle.row_start_center, la: "flex-start", flexWrap: 'wrap',

            }}>
                {data.map(item => (
                    <Pressable style={{ ...commonStyle.row_space_arround_center, marginRight: 15 }}
                        onPress={() => {
                    ...
                    >
                        ...
                        <Icon name={getStatus(values, item) == "checked" ? "checkbox-marked" : "checkbox-blank-outline"}
                            size={25}
                            color={colors.primary}

                        />
                        <Text variant="labelMedium" style={{ margin: 10, alignSelf: "baseline", fontSize: 17 }}>{item.description}</Text>
                    </Pressable>
...
const setStatus = (selected, item) => {
    ...
    let values = selected
    const data = { code: item.code, description: item.description, active: !item.active }
    if (get(values, 'length', 0) == 0) {
        return [data];
    }
